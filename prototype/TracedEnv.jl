...
mutable struct Trace{T<:Real}
    puck_trace::Vector{Vector{T}}
    mallet1_trace::Vector{Vector{T}}
    mallet2_trace::Vector{Vector{T}}
end
function update_positions!(
    ...
    push!(trace.puck_trace, copy(puck.pos))
    push!(trace.mallet1_trace, mallet1 !== nothing ? copy(mallet1.pos) : Vector{V}())
    push!(trace.mallet2_trace, mallet2 !== nothing ? copy(mallet2.pos) : Vector{V}())
end
...
function simulate_dt!(env::AirHockeyEnv)
    ...
    trace = Trace{Float32}(Vector{Vector{Float32}}(), Vector{Vector{Float32}}(),Vector{Vector{Float32}}())
    while t_remain > 0
        ...
        update_positions!(puck, mallet1, mallet2, t_next, trace) # przewiń symulacje do momentu kolizji
